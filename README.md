...
当前仓库主要包含 Rust 核心模型、Windows 平台信号层、D3D11/HLSL 黑洞渲染器和若干 demo。摆件模式已提供托盘退出、拖动锁定和连续参数面板；安装包等完整产品外壳还在后续规划中。
...
```powershell
cargo run --example ornament_demo
```

`ornament_demo` 不在任务栏显示，会常驻托盘；右键托盘图标可以打开参数面板、锁定/解锁拖动、切换截图可见模式或退出。黑洞会小范围自动移动；未锁定时可以拖动事件视界，锁定后点击会穿透到后面的窗口。参数面板支持连续调整视界大小、视界质量、爱因斯坦环范围、吸积盘开关、吸积盘强度、颜色、倾角和范围；吸积盘勾选后立即出现，半径会随黑洞增大而扩张。

默认情况下，摆件窗口会从系统屏幕捕获中排除，避免内部 WGC 捕获把黑洞自身采进去形成反馈环。需要截图或录屏时，可在托盘菜单开启“截图可见”；此时内部捕获会冻结在最近一帧，系统截图能截到黑洞。截图后选择“恢复实时捕获”即可继续实时透镜。
...
- 摆件模式 demo：托盘退出、拖动锁定、参数面板、自动微动和手动拖动。
- 单元测试与集成测试。
